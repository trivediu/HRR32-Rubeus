...
const getOfficials = function(region, dataSet = data) {
  ...
  const officeIndices = [];
  const officeTitles = [];
  getOfficeIndicesAndTitlesFromData(officeIndices, officeTitles, searchSpace);

  const regionOfficials = [];
  getOfficialsFromData(officeIndices, regionOfficials);
  addTitleToOfficials(regionOfficials, officeTitles);
